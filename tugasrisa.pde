...
void draw() {
  background(#CB7417);
  fill(#864C0E); //warnaular
  for (int i = 0; i< x.size(); i++) rect(x.get(i)*blocks, y.get(i)*blocks, blocks, blocks);
  if(!gameover){
    fill(fc1, fc2, fc3); //warna makanan ular
    ellipse(foodx*blocks+10, foody*blocks, blocks, blocks); //bentuk makanan
    textAlign(LEFT); //score permainan
    textSize(25);
    fill(255);
